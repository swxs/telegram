...
export class TelegramBridge {
    ...
    start() {
        ...
        queueMicrotask(() => {
            if (this.stopped || this.disposeInteractions !== undefined)
                return;
        ...
        if (this.registerQuestionProvider) {
            try {
                ...
                if (isDuplicateProviderError(error)) {
                    this.ctx.logger.info('[telegram] user-questions provider already registered (web host); skipping Telegram question UI');
                }
                else {
                    throw error;
                }
