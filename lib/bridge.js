...
export class TelegramBridge {
        ...
        return {
            ...
            setup: async (agentCtx) => {
                ...
                try {
                    ...
                    this.ctx.logger.warn('[telegram] tele_ask_user wiring failed (session still usable): %s', messageOf(error));
                }
        ...
        return () => {
            ...
            for (const pending of [...this.pendingById.values()]) {
                if (pending.kind === 'question')
                    ...
                    void this.settleApproval(pending, 'cancelled');
            }
        };
    }
    ...
    handleApprovalRequest(req, next) {
        ...
        try {
            ...
            if (downstream !== undefined && typeof downstream.then === 'function') {
    ...
    async dismissApprovalPending(pending, suffix) {
