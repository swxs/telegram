...
**Slash Command**:
A user message that starts with `/` and is handled by the bridge; it is not forwarded to the agent. User-facing commands are `/start`, `/clear`, `/skills`, and `/help`. `/init` is an Init Admin command; it is not listed in `/help` or the Command Menu. A `//name` message is rewritten to `/name` and forwarded to the agent as a Skill invoke. Any other `/` message is an unknown command.
...
**Skill**:
A kebab-case instruction pack discovered for the bound Workspace. `/skills` lists user-invocable Skill names in two columns (20 per page); tapping a name copies `//name ` so the user can paste, edit, and send it. The bridge rewrites a leading `//` to `/` before the agent sees the message.
_Avoid_: tool, prompt, command pack
