...
- **宿主前置条件**：dsh 组合必须挂载 `agents`、`agentPresets`、`userQuestions`（`@deepseek-ai/dsh-user-questions`）与 `approval`（`@deepseek-ai/dsh-user-approval`）；要让模型能发起提问还需 `tool-ask-user`，计划审核还需 `plan-mode`。LLM 适配器、会话来自外围 `cordis.yml`（见 [`telegram-agent`](examples/telegram-agent/README.zh.md) 示例）。缺少任一必需服务时插件加载失败。
